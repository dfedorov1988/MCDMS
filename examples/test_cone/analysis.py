...
#     Plotting eigenstate energies only for first trajectory for clarity
    ...
    for n_state in range(numstates):
        ...
        plt.plot(time[key][::10], poten[key][::10, n_state],
                 color=colors[n_state], label=label,
                 linestyle=linestyles[n_key], marker=markers[n_key],
...
def plot_approx_energies(keys, numstates, colors, linetypes, markers):
    ...
    for key in keys:
        ...
        for n_state in range(numstates):
            if n_key == 0:
                cur_label = str(n_state + 1) + " state"
            else:
                cur_label = None
            plt.plot(
                time[key][::10], approx_e[::10, n_state],
                color=colors[n_state], label=cur_label,
    ...
    for key in keys:
        plt.plot(time[key], aven[key], color='black',
                 label=key, linestyle=linestyles[n_key], marker=markers[n_key],
                 markersize=3, markevery=20)
        n_key += 1
...
plot_total_energies(an.datasets["labels"], len(an.datasets["labels"]),
                    colors, linestyles)
plot_real_wf(an.datasets["labels"], nstates, krylov_sub_n)
plot_approx_wf(an.datasets["labels"], nstates, krylov_sub_n)
# plot_approx_el_population(an.datasets["labels"], krylov_sub_n, colors,
# linestyles, markers)
